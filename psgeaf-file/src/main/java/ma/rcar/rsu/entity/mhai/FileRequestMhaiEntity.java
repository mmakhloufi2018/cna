...
@AllArgsConstructor
@NoArgsConstructor
@Entity(name = "FILE_REQUEST_MHAI")
public class FileRequestMhaiEntity implements Serializable {
    ...
    private List<DetailsFileRequestMhaiEntity> lignes;
    ...
    public static FileRequestMhaiEntity from(FileRequestMhaiDto in) {
        if (in == null)
            return null;
        FileRequestMhaiEntity out = new FileRequestMhaiEntity();
        ...
        if (in.getDetails() != null) {
            out.setLignes(in.getDetails().stream().map(DetailsFileRequestMhaiEntity::from).collect(Collectors.toList()));
        }
        return out;
    }
}
