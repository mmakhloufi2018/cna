...
@Entity
@Table(name = "FILE_REQUEST_ONOUSC_INSCRIP")
@Getter
@Setter
@AllArgsConstructor
@NoArgsConstructor
public class FileRequestOnouscInscriptionEntity implements Serializable {
	...
	private List<DetailsFileRequestOnouscInscriptionEntity> lignes;
	...
	public static FileRequestOnouscInscriptionEntity from(FileRequestInscriptionDto in) {
		if (in == null)
			return null;
		FileRequestOnouscInscriptionEntity out = new FileRequestOnouscInscriptionEntity();
		...
		if (in.getDetails() != null) {
			out.setLignes(in.getDetails().stream().map(DetailsFileRequestOnouscInscriptionEntity::from).collect(Collectors.toList()));
		}
		return out;
	}
