...
@AllArgsConstructor
@NoArgsConstructor
@Entity(name = "FILE_REQUEST_FEF")
public class FileRequestFefEntity {
    ...
    private List<DetailsFileRequestFefEntity> lignes;
    ...
    public static FileRequestFefEntity from(FileRequestFefDto in) {
        if (in == null)
            return null;
        FileRequestFefEntity out = new FileRequestFefEntity();
        ...
        if (in.getDetails() != null) {
            out.setLignes(in.getDetails().stream().map(DetailsFileRequestFefEntity::from).collect(Collectors.toList()));
        }
        return out;
    }
}
